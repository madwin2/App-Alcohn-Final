...
import { VECTORIZAR_ONBOARDING, type OnboardingSlide } from '@/lib/vectorizacion/onboarding';
import { TourVisual, type TourVisualId } from '@/components/vectorizacion/tour/TourVisuals';

interface VectorizarTourDialogProps {
  open: boolean;
  onClose: () => void;
...
type Page =
  ...
  | { key: string; kind: 'feature'; heading: string; body: string; visual: TourVisualId };

/**
 * Tour visual de Vectorizar: panel fijo (media | copy) en todas las slides.
 */
export function VectorizarTourDialog({
  open,
  onClose,
  slides = [...VECTORIZAR_ONBOARDING.slides],
  introHeading = VECTORIZAR_ONBOARDING.introHeading,
  introBody = VECTORIZAR_ONBOARDING.introBody,
  introImage = VECTORIZAR_ONBOARDING.introImage,
}: VectorizarTourDialogProps) {
      ...
      <DialogPortal>
        ...
        >
          ...
          <div className="grid min-h-0 flex-1 grid-rows-[minmax(11rem,1fr)_auto] md:grid-cols-[1.15fr_0.85fr] md:grid-rows-none">
            <div className="relative min-h-0 overflow-hidden border-b border-white/5 md:border-b-0 md:border-r">
              {page.kind === 'intro' ? (
                <img
                  key={page.key}
                  src={page.image}
                  alt=""
                  className="absolute inset-0 h-full w-full object-cover object-center vt-media-in"
                />
              ) : (
                <TourVisual id={page.visual} />
              )}
            </div>
