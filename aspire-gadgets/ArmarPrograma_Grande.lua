-- VECTRIC LUA SCRIPT
-- Armar Programa - Maquina Grande (G) -- TODO EN UN SOLO ARCHIVO
--
...
--   2. Correr este gadget (Toolpaths > Gadgets > Armar Programa Grande).
...
local MACHINE_LABEL = "Grande"
local MACHINE_CODE = "G"
...
-- Posiciones de columnas (identicas a AUTOMATIZACION_SIMPLIFICADA GRANDE.lua
-- / AUTOMATIZACION_SIMPLIFICADA_3MM GRANDE.lua originales).
local COLUMNS = {
  { nom = 38,   eff = 37,   xpos = 50.65 },
  { nom = 12.7, eff = 11.7, xpos = 94.4 },
}
local FALLBACK_COLUMN_INDEX = 1 -- la columna 38mm es la mas grande para esta maquina
local TARGET_TOP_Y = -9.788
...
local function createAdditionalElements(column, bbox)
...
local function processSingleObject(obj)
  ...
  if not candidate then
    candidate = COLUMNS[FALLBACK_COLUMN_INDEX]
    needRotation = false
  end
