/**
 * Versión simplificada del logo (misma identidad, trazo más limpio) vía OpenAI Images edits.
 * Misma pila que optimize-logo.js; solo cambia el prompt.
 */
...
const SIMPLIFY_PROMPT = [
  'Simplify this logo into a cleaner stamp-ready version while keeping the SAME logo identity, same words/symbols, and same overall layout.',
  'Reduce fine noise, hairlines, and tiny details that would not reproduce well as a stamp; keep shapes bold and readable.',
  'Do NOT change the brand concept, do NOT replace text, do NOT add new elements, do NOT invent a different logo.',
  'Output one centered logo on transparent or pure white background, monochrome black ink, high contrast, crisp edges.',
  'No shadows, no gradients, no textures, no decorations.',
].join(' ');
...
async function callEditMultipart({ apiKey, model, inputBuffer, mime, fieldName, signal }) {
  ...
  form.append('prompt', SIMPLIFY_PROMPT);
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    res.status(503).json({ error: 'AI simplify not configured', ok: false });
    return;
  }
  ...
  try {
    for (const model of models) {
      ...
      if (result.ok && result.optimizedDataUrl) {
        res.status(200).json({
          ...
          source: `openai-simplify:${result.model}`,
    ...
    res.status(200).json({
      ok: false,
      error: 'OpenAI simplify failed for all models',
    ...
    res.status(500).json({
      ok: false,
      error: 'Unexpected simplify error',
